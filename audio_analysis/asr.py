...
class AutoSpeechRecognition():
    ...
    """
    def __init__(self,lib="stable_whisper", model="base"):
        """
    ...
    def __load_lib(self,lib,model):
        ...
        if lib=="whisper":
            ...
            self.recognizer = stable_whisper.load_model(model)
    ...
    def get_transcript(self, timestamp=False, minimum_sentence_length=15,max_sentence_length=70,clip_duration_percentage=0.1):
        """
        ...
        if self.__lib =="whisper":
            transcript = self.__get_transcript__whisper(timestamp, clip_duration_percentage=clip_duration_percentage)

        elif self.__lib =="stable_whisper":
            transcript = self.__get_transcript_stable_whisper(minimum_sentence_length,max_sentence_length=max_sentence_length)
    ...
    def __get_transcript__whisper(self, timestamp=False,clip_duration_percentage=0.1):
        """
        ...
        transcript = {}
        if not timestamp:
            transcript["text"] = self.recognizer.transcribe(self.media_path)
            return transcript
        else:
            ...
            try:
                ...
                for i,stamps in enumerate(timestamps_lis):
                    ...
                    result = self.recognizer.transcribe(audio_data,condition_on_previous_text=transcript)
    ...
    def __get_transcript_stable_whisper(self,minimum_sentence_length,max_sentence_length):
        """
        ...
        """
        transcript = {}
        result = self.recognizer.transcribe(self.media_path)
    ...
    def __generate_sentence_level_transcript(self,word_timestamps, minimum_sentence_length=10,max_sentence_length=70, skip_punctuations=["Dr.", "Mr.", "Mrs.", "Ms.",'Jr.',"Sr."]):
            ...
            for i,word_timing in enumerate(word_timestamps):
                ...
                if  len(current_sentence_words)>=max_sentence_length or (word_timing.word.endswith(('.', '!', '?')) and word_timing.word not in skip_punctuations):
