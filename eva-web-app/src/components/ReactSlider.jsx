// all credit to https://github.com/zillow/react-slider
...
class ReactSlider extends React.Component {
    ...
    onSliderClick = e => {
        ...
        if (this.props.onSliderClick && !this.hasMoved) {
            const position = this.getMousePosition(e);
            // console.log("POsition in slider:", position)

            const calcOffsetPos = this.calcOffsetFromPosition(position[0])

            // console.log("Calc offset:", calcOffsetPos)

            const calcVal = this.calcValue(calcOffsetPos)

            // console.log("Calc val:", calcVal)
    ...
    resize() {
        ...
        if (!slider || !thumb) {
            // console.log("Slider or thumb is null, returning")
            return;
        }
    ...
    calcOffsetFromPosition(position) {
        ...
        // console.log("Rect:", sliderRect, "max:", sliderMax, "min:", sliderMin)
        ...
        // console.log("Pixel offset before division:", pixelOffset)
        if (this.state.thumbSize != null) {
        ...
        // console.log("Thumb size:", this.state.thumbSize)
        // console.log("pixel offset after division:", pixelOffset)
        return pixelOffset;
    }
    ...
    renderTracks(offset) {
        ...
        if (offset.length < 1) {
            tracks.push(this.renderTrack(0,0,0))
            return tracks
        }
